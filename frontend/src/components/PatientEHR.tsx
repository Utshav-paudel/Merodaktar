    ...
    return (
      <AppLayout role="patient" user={user} onLogout={onLogout} title="Health Records">
        <div className="flex min-h-[60vh] flex-col items-center justify-center gap-4">
          ...
          <p className="text-sm text-slate-500">Loading your health records…</p>
        </div>
      </AppLayout>
    );
  }
  ...
  return (
    <AppLayout role="patient" user={user} onLogout={onLogout} title="Health Records">
      <div className="animate-fade-in-up space-y-8">
        {/* Hero header */}
        <div className="relative overflow-hidden rounded-3xl border border-slate-200 bg-gradient-to-br from-brand-50 via-white to-accent-50 p-6 shadow-soft sm:p-8">
          <div className="absolute -right-16 -top-16 h-48 w-48 rounded-full bg-brand-100/60 blur-3xl" />
          <div className="absolute -bottom-16 left-1/3 h-48 w-48 rounded-full bg-accent-100/50 blur-3xl" />
        ...
        {/* Tabs */}
        <div className="flex flex-wrap gap-2 rounded-2xl border border-slate-200 bg-white p-1.5 shadow-soft">
          {TABS.map((tab) => (
            <button
              key={tab.key}
        ...
        {activeTab === 'overview' && (
          <div className="space-y-6">
                ...
                <div className="grid grid-cols-2 gap-4 md:grid-cols-4">
                  ...
                  {summary.latest_vitals.heart_rate && (
                    <div className="rounded-xl border border-slate-200 bg-slate-50 p-4">
                      ...
                      <p className="mt-1 text-xl font-bold text-slate-900">{summary.latest_vitals.heart_rate} bpm</p>
                    </div>
                  )}
                  {summary.latest_vitals.weight && (
                    <div className="rounded-xl border border-slate-200 bg-slate-50 p-4">
                      <p className="text-xs font-medium text-slate-500">Weight</p>
                      <p className="mt-1 text-xl font-bold text-slate-900">{summary.latest_vitals.weight} kg</p>
                    </div>
                  )}
                  {summary.latest_vitals.bmi && (
                    <div className="rounded-xl border border-slate-200 bg-slate-50 p-4">
                      <p className="text-xs font-medium text-slate-500">BMI</p>
                      <p className="mt-1 text-xl font-bold text-slate-900">{summary.latest_vitals.bmi}</p>
                    </div>
                  )}
                </div>
              ) : (
                <p className="py-4 text-center text-sm text-slate-400">No vital signs recorded yet</p>
              )}
            </Card>
            ...
            {(!summary || (summary.active_medications_count === 0 && summary.allergies_count === 0 && summary.chronic_conditions_count === 0)) && (
              <Card className="border-brand-200 bg-brand-50 p-6 text-center">
            ...
            <div className="space-y-3">
              {ehrData.vital_signs && ehrData.vital_signs.length > 0 ? (
                ehrData.vital_signs.map((vital: any, index: number) => (
                  <div key={index} className="rounded-xl border border-slate-200 bg-slate-50 p-4 transition hover:bg-slate-100">
                    ...
                    <div className="grid grid-cols-2 gap-3 text-sm text-slate-600 md:grid-cols-4">
                      {vital.blood_pressure_systolic && (
                        <div>BP: <span className="font-semibold text-slate-900">{vital.blood_pressure_systolic}/{vital.blood_pressure_diastolic}</span> mmHg</div>
                      )}
                      {vital.heart_rate && <div>HR: <span className="font-semibold text-slate-900">{vital.heart_rate}</span> bpm</div>}
                      {vital.temperature && <div>Temp: <span className="font-semibold text-slate-900">{vital.temperature}°C</span></div>}
                      {vital.weight && <div>Weight: <span className="font-semibold text-slate-900">{vital.weight}</span> kg</div>}
                      {vital.bmi && <div>BMI: <span className="font-semibold text-slate-900">{vital.bmi}</span></div>}
                    </div>
                  </div>
                ))
              ) : (
        ...
        {activeTab === 'medications' && ehrData && (
          <Card className="p-6">
            <div className="mb-5 flex items-center justify-between">
              <h3 className="flex items-center gap-2 font-display text-lg font-semibold text-slate-900">
                <BeakerIcon className="h-5 w-5 text-accent-500" />
                Current Medications
              </h3>
            ...
            <div className="space-y-3">
              {ehrData.medications && ehrData.medications.length > 0 ? (
                ehrData.medications.map((med: any, index: number) => (
                  <div key={med.id || index} className="rounded-xl border border-slate-200 bg-slate-50 p-4 transition hover:bg-slate-100">
                    <div className="flex items-start justify-between gap-3">
                      <div>
                        <h4 className="font-semibold text-slate-900">{med.name || 'Medication'}</h4>
                        <p className="text-sm text-slate-500">{med.dosage} {med.frequency && `- ${med.frequency}`}</p>
                        {med.start_date && (
                        ...
                        {med.prescribed_by && (
                          <p className="text-xs text-slate-400">Prescribed by: {med.prescribed_by}</p>
                        )}
                      </div>
            ...
            <div className="space-y-3">
              {ehrData.allergies && ehrData.allergies.length > 0 ? (
                ehrData.allergies.map((allergy: any, index: number) => (
                  <div key={allergy.id || index} className="rounded-xl border border-slate-200 bg-slate-50 p-4 transition hover:bg-slate-100">
                    <div className="flex items-start justify-between gap-3">
                      <div>
                        <h4 className="font-semibold text-slate-900">{allergy.allergen || allergy.name || 'Allergen'}</h4>
                        <p className="text-sm text-slate-500">Reaction: {allergy.reaction || 'Not specified'}</p>
        ...
        {activeTab === 'history' && ehrData && (
          <Card className="p-6">
            <div className="mb-5 flex items-center justify-between">
              <h3 className="flex items-center gap-2 font-display text-lg font-semibold text-slate-900">
                <ClipboardDocumentListIcon className="h-5 w-5 text-brand-600" />
            ...
            <div className="space-y-3">
              {ehrData.chronic_conditions && ehrData.chronic_conditions.map((history: any, index: number) => (
                <div key={index} className="rounded-xl border border-slate-200 bg-slate-50 p-4 transition hover:bg-slate-100">
                  <div className="flex items-start justify-between gap-3">
                    <div>
                      <h4 className="font-semibold text-slate-900">{history.condition || history.name || 'Condition'}</h4>
                      <p className="text-sm text-slate-500">
                      ...
                      {history.notes && (
                        <p className="mt-1 text-sm text-slate-400">{history.notes}</p>
                      )}
                    </div>
        ...
        {activeTab === 'encounters' && (
          <Card className="p-6">
            <h3 className="mb-5 flex items-center gap-2 font-display text-lg font-semibold text-slate-900">
            ...
            {encounters.length === 0 ? (
              ...
              <div className="relative space-y-6 pl-6 before:absolute before:left-2 before:top-2 before:bottom-2 before:w-px before:bg-slate-200">
                {encounters.map((encounter: any) => (
                  <div key={encounter.id} className="relative">
                    <span className="absolute -left-[1.45rem] top-1.5 h-3 w-3 rounded-full bg-gradient-brand ring-4 ring-white" />
                    <div className="rounded-2xl border border-slate-200 bg-white p-5 shadow-soft">
                      <div className="mb-3 flex items-start justify-between gap-3">
                        <div className="flex-1">
                          {/* Encounter Date as Header */}
                          <div className="mb-2 flex flex-wrap items-center gap-3">
                            <h4 className="flex items-center gap-2 font-display text-lg font-bold text-slate-900">
                          ...
                          <p className="text-lg font-semibold text-slate-900">{encounter.chief_complaint}</p>
                            ...
                            {typeof encounter.patient_summary === 'object' ? (
                              <>
                                <div className="grid grid-cols-2 gap-2">
                                  <p><strong className="text-slate-700">Name:</strong> {encounter.patient_summary.patient_name}</p>
                                  <p><strong className="text-slate-700">Age:</strong> {encounter.patient_summary.age}</p>
                                  <p><strong className="text-slate-700">Gender:</strong> {encounter.patient_summary.gender}</p>
                                  <p><strong className="text-slate-700">Blood Type:</strong> {encounter.patient_summary.blood_type || 'Unknown'}</p>
                                  <p><strong className="text-slate-700">Phone:</strong> {encounter.patient_summary.phone}</p>
                                  <p><strong className="text-slate-700">Address:</strong> {encounter.patient_summary.address}</p>
                                </div>
                                ...
                                {encounter.patient_summary.vital_signs && Object.keys(encounter.patient_summary.vital_signs).length > 0 && (
                                  <div className="mt-3 border-t border-slate-200 pt-3">
                                    <p className="mb-2 flex items-center gap-1.5 font-semibold text-slate-700">
                                      <HeartIcon className="h-4 w-4 text-rose-500" /> Latest Vital Signs
                                    </p>
                                    <div className="grid grid-cols-3 gap-2 text-xs">
                                      {encounter.patient_summary.vital_signs.blood_pressure && encounter.patient_summary.vital_signs.blood_pressure !== 'Not recorded' && (
                                        <p><strong className="text-slate-700">BP:</strong> {encounter.patient_summary.vital_signs.blood_pressure}</p>
                                      )}
                                      {encounter.patient_summary.vital_signs.heart_rate && encounter.patient_summary.vital_signs.heart_rate !== 'Not recorded' && (
                                        <p><strong className="text-slate-700">HR:</strong> {encounter.patient_summary.vital_signs.heart_rate}</p>
                                      )}
                                      {encounter.patient_summary.vital_signs.temperature && encounter.patient_summary.vital_signs.temperature !== 'Not recorded' && (
                                        <p><strong className="text-slate-700">Temp:</strong> {encounter.patient_summary.vital_signs.temperature}</p>
                                      )}
                                      {encounter.patient_summary.vital_signs.weight && encounter.patient_summary.vital_signs.weight !== 'Not recorded' && (
                                        <p><strong className="text-slate-700">Weight:</strong> {encounter.patient_summary.vital_signs.weight}</p>
                                      )}
                                      {encounter.patient_summary.vital_signs.height && encounter.patient_summary.vital_signs.height !== 'Not recorded' && (
                                        <p><strong className="text-slate-700">Height:</strong> {encounter.patient_summary.vital_signs.height}</p>
                                      )}
                                      {encounter.patient_summary.vital_signs.bmi && encounter.patient_summary.vital_signs.bmi !== 'Not calculated' && (
                                        <p><strong className="text-slate-700">BMI:</strong> {encounter.patient_summary.vital_signs.bmi}</p>
                                      )}
                                    </div>
                                  </div>
                                )}
                                ...
                                {encounter.patient_summary.current_medications && encounter.patient_summary.current_medications.length > 0 && (
                                  <div className="mt-3 border-t border-slate-200 pt-3">
                                    <p className="mb-2 flex items-center gap-1.5 font-semibold text-slate-700">
                                      <BeakerIcon className="h-4 w-4 text-accent-500" /> Current Medications
                                    </p>
                                    <ul className="list-inside list-disc space-y-1 text-xs">
                                      {encounter.patient_summary.current_medications.map((med: any, idx: number) => (
                                        <li key={idx}>
                                          <strong className="text-slate-700">{med.name}</strong> - {med.dosage} ({med.frequency})
                                        </li>
                                      ))}
                                    </ul>
                                  </div>
                                ...
                                {encounter.patient_summary.allergies && encounter.patient_summary.allergies.length > 0 && (
                                  <div className="mt-3 border-t border-slate-200 pt-3">
                                    <p className="mb-2 flex items-center gap-1.5 font-semibold text-slate-700">
                                      <ExclamationTriangleIcon className="h-4 w-4 text-rose-500" /> Allergies
                                    </p>
                                    <ul className="list-inside list-disc space-y-1 text-xs">
                                      {encounter.patient_summary.allergies.map((allergy: any, idx: number) => (
                                        <li key={idx} className="text-rose-600">
                                ...
                                {encounter.patient_summary.medical_history && encounter.patient_summary.medical_history.length > 0 && (
                                  <div className="mt-3 border-t border-slate-200 pt-3">
                                    <p className="mb-2 flex items-center gap-1.5 font-semibold text-slate-700">
                                      <ClipboardDocumentListIcon className="h-4 w-4 text-brand-600" /> Medical History
                                    </p>
                                    <ul className="list-inside list-disc space-y-1 text-xs">
                                      {encounter.patient_summary.medical_history.map((condition: any, idx: number) => (
                                        <li key={idx}>
                                          <strong className="text-slate-700">{condition.condition}</strong> - Diagnosed: {condition.diagnosed_date} ({condition.status})
                                        </li>
                                      ))}
                                    </ul>
                                  </div>
                      ...
                      {encounter.ai_preliminary_report && (
                        <div className="mt-3 rounded-xl border border-brand-200 bg-brand-50 p-4">
                          ...
                          <p className="whitespace-pre-line text-sm text-slate-600">{encounter.ai_preliminary_report}</p>
                        </div>
                      )}

                      {encounter.symptoms && encounter.symptoms.length > 0 && (
                        <div className="mt-3">
                          <p className="mb-2 text-sm font-semibold text-slate-600">Symptoms</p>
                      ...
                      {encounter.assessment && (
                        <div className="mt-3 rounded-xl border border-amber-200 bg-amber-50 p-4">
                          <p className="mb-2 text-sm font-semibold text-amber-700">Assessment</p>
                          <p className="text-sm text-slate-600">{encounter.assessment}</p>
                        </div>
                      )}

                      {/* Doctor Notes - Always show with placeholder if empty */}
                      <div className="mt-3 rounded-xl border border-emerald-200 bg-emerald-50 p-4">
                        <p className="mb-2 text-sm font-semibold text-emerald-700">Doctor Notes</p>
                        {encounter.doctor_notes && encounter.doctor_notes.trim() !== '' ? (
                          <p className="whitespace-pre-line text-sm text-slate-600">{encounter.doctor_notes}</p>
                        ) : encounter.appointment_id ? (
                          <p className="text-sm italic text-slate-400">Doctor notes will be added after your appointment.</p>
                        ) : (
                          <p className="text-sm italic text-slate-400">No appointment booked yet. Book an appointment to consult with a doctor.</p>
                        )}
                      </div>
                          ...
                          <div className="grid grid-cols-2 gap-3 md:grid-cols-4">
                            {encounter.vital_signs.blood_pressure_systolic && (
                              <div className="text-sm">
                                ...
                                <span className="ml-1 font-semibold text-slate-900">{encounter.vital_signs.blood_pressure_systolic}/{encounter.vital_signs.blood_pressure_diastolic}</span>
                              </div>
                            )}
                            {encounter.vital_signs.heart_rate && (
                              <div className="text-sm">
                                <span className="font-medium text-slate-500">HR:</span>
                                <span className="ml-1 font-semibold text-slate-900">{encounter.vital_signs.heart_rate} bpm</span>
                              </div>
                            )}
                            {encounter.vital_signs.temperature && (
                              <div className="text-sm">
                                <span className="font-medium text-slate-500">Temp:</span>
                                <span className="ml-1 font-semibold text-slate-900">{encounter.vital_signs.temperature}°C</span>
                              </div>
                            )}
                            {encounter.vital_signs.respiratory_rate && (
                              <div className="text-sm">
                                <span className="font-medium text-slate-500">RR:</span>
                                <span className="ml-1 font-semibold text-slate-900">{encounter.vital_signs.respiratory_rate}/min</span>
                              </div>
                            )}
                          </div>
                        </div>
      ...
      {showAddModal && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-slate-900/40 p-4 backdrop-blur-sm">
          <Card className="w-full max-w-md p-6 animate-fade-in-up">
            <h3 className="mb-5 font-display text-xl font-semibold text-slate-900">
